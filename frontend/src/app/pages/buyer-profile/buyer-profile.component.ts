...
export class BuyerProfileComponent implements OnInit {
  protected readonly buyer = signal<User | null>(null);
  ...
  async ngOnInit() {
    const buyerId = localStorage.getItem('vibebuy_buyer_id');
    if (!buyerId) {
      this.status.set('No buyer profile yet.');
      this.loading.set(false);
      return;
    }

    try {
      const buyer = await firstValueFrom(this.api.getUser(buyerId));
      this.buyer.set(buyer);
      this.name.set(buyer.name);
      this.city.set(buyer.city ?? '');
      this.avatarUrl.set(buyer.avatarUrl ?? '');
    } catch {
      this.status.set('Unable to load buyer profile.');
    } finally {
      this.loading.set(false);
    }
  }
  ...
  async save() {
    ...
    try {
      const updated = await firstValueFrom(
        this.api.updateUser(buyer.id, {
          name: this.name(),
          city: this.city(),
          avatarUrl: this.avatarUrl(),
        })
      );
      this.buyer.set(updated);
